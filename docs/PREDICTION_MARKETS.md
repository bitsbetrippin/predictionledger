...
## Release candidate 2.0 (2.0.0-rc.1)

A review of the whole execution path fixed fourteen findings (two of them P1: the armed strategy/category was not enforced, and a second process could corrupt an in-flight submission) plus the one that would have blocked every real decision (the freshness clock ran ahead of the fetched book). The upgrade from an authentic 1.9.0 database is a rehearsal you run yourself (`npm run upgrade:rehearse -- <db>`) and a test (O05); the secret key file gets a verified Windows ACL; the paper-soak and qualification reports (Setup → Automatic execution, `npm run report:*`) say **pending** until seven real days and 100 distinct settled real events exist. That is why this is a release candidate: the owner-only evidence — Windows on real content, the capped smoke test, the real soak, a production qualification, the rehearsal on your real data, the first real settlement — is listed in VERIFICATION.md and none of it is waived. ADR-036 has the rules; SETUP §4.16 is the owner list.
