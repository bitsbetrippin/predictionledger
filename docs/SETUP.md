...
Sports picks are settled with the **Validate scores** button (Predictions → detail panel), which looks up the final score from trusted sources — league sites, ESPN, AP, CBS/Fox/NBC/Yahoo Sports, BBC/Sky, the Reference sites — and records Hit / Miss / Push, or *No final score yet* if the game has not been played or reported. It is refused before the game date. When the transcript never says the date ("Week 1", "Sunday night"), the button first looks the game up on a published schedule (two searches, trusted sites first) and records where the date came from — you will see *date from schedule look-up* with a source link in the detail panel, and the deadline basis reads `lookup`. If no schedule page states the date, the job fails with a message asking you to set the deadline by hand; the app never guesses one. No validation plan review or deep research is involved; the plan is written by the app.
