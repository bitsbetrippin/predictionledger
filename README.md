...
> **Status: 1.14.0.** Feature-complete against the original specification (releases 0.1 → 1.1), the sports rule (1.2 → 1.4), the prediction-market plan (1.5 → 1.9, see [docs/PREDICTION_MARKETS.md](docs/PREDICTION_MARKETS.md)), and the first four releases of the Polymarket US track: 1.10 (account connection and read-only US market data), 1.11 (source subscriptions, provenance, an execution-specific contract verifier — [ADR-032](docs/DECISIONS.md)), 1.12 (immutable forecasts, pure trade/no-trade decisions with atomic reservations, an execution-aware US paper engine — [ADR-033](docs/DECISIONS.md)) 1.13 (**manual-live execution**: preview → one confirmed, bounded IOC order → private-stream and REST reconciliation → official settlement — [ADR-034](docs/DECISIONS.md)) and 1.14 (**automatic execution behind explicit arming**: a scheduler that matches, verifies and decides on its own timer and — only while armed against the policy hash you reviewed and a production-qualified strategy — places bounded orders through the same one-send path; pause, emergency stop, circuit breaker, incident alerts and the Trades ledger — [ADR-035](docs/DECISIONS.md)). **This build can place a real order in two ways: manual-live (preview → confirm) or automatic while armed. Arming requires a production strategy qualification that does not exist yet, so on a fresh install the Arm control explains why it is unavailable.** 170 automated tests, all against the fake venue (the pinned SDK is exercised with a stubbed `fetch`); **no production order was placed during development** — the capped owner-run smoke test ([docs/SETUP.md §4.14](docs/SETUP.md)) and the read-only account check are **pending owner execution**, as are the 1.11–1.13 exit demos on the owner's machine. A production strategy qualification (≥ 100 settled events with a market baseline, evaluated on real data — fixtures never count) does not exist yet, so automation cannot be armed; the 1.14 owner acceptance list is [docs/SETUP.md §4.15](docs/SETUP.md). What has actually been executed on which platform is tracked in [docs/VERIFICATION.md](docs/VERIFICATION.md) and [docs/FIRST_RUN.md](docs/FIRST_RUN.md) — releases 1.4 → 1.14 have been tested against fixtures and live public venue APIs, not yet end to end on real videos, so expect the parsers, matchers, the contract checklist and the cohort to need tuning on the first real run. Release-by-release detail: [CHANGELOG.md](CHANGELOG.md).
...
│       │                     dispatchLease, autoTrader (scheduler: discover → decide → one send), tradingAlerts (incident-keyed), ledger (Trades summary / rows / export), backup, export
...
│       └── **/*.test.ts      node:test suites (170): core, analysis, sports, markets, signals, watch/consensus, paper, trading (fake venue; the pinned SDK with stubbed fetch), provenance, contracts, forecasts, decisions, execution (E01–E12 with fault injection), automation (U01–U06, D01/D03, O03/O04), autopilot end-to-end (fake source → order), media, youtube, pipelines
...
│       └── components/       PredictionDetail (quote, components, plan, evidence, dossier, history, Markets tab) · MarketLinks · ContractPanel · DossierView · SubscriptionsCard · PolymarketUsCard · TradingLimitsCard · AutomationCard
...
|---|---|---|
| **2.0.0** | Upgrade/restore drills, Windows verification on real content, a real paper soak and the production qualification evidence that unlocks arming. | No open P0/P1; insufficient calibration data = auto-live gate unmet (it is unmet today). |
