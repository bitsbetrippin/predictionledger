...
> **Status: 1.13.0.** Feature-complete against the original specification (releases 0.1 → 1.1), the sports rule (1.2 → 1.4), the prediction-market plan (1.5 → 1.9, see [docs/PREDICTION_MARKETS.md](docs/PREDICTION_MARKETS.md)), and the first four releases of the Polymarket US track: 1.10 (account connection and read-only US market data), 1.11 (source subscriptions, provenance, an execution-specific contract verifier — [ADR-032](docs/DECISIONS.md)), 1.12 (immutable forecasts, pure trade/no-trade decisions with atomic reservations, an execution-aware US paper engine — [ADR-033](docs/DECISIONS.md)) and 1.13 (**manual-live execution**: preview → one confirmed, bounded IOC order → private-stream and REST reconciliation → official settlement — [ADR-034](docs/DECISIONS.md)). **This build can place a real order, only in manual-live mode after a typed owner acknowledgement, and only through preview → confirm; nothing is automated.** 161 automated tests, all against the fake venue (the pinned SDK is exercised with a stubbed `fetch`); **no production order was placed during development** — the capped owner-run smoke test ([docs/SETUP.md §4.14](docs/SETUP.md)) and the read-only account check are **pending owner execution**, as are the 1.11–1.13 exit demos on the owner's machine. A production strategy qualification (≥ 100 settled paper decisions) does not exist yet, so automation stays gated. What has actually been executed on which platform is tracked in [docs/VERIFICATION.md](docs/VERIFICATION.md) and [docs/FIRST_RUN.md](docs/FIRST_RUN.md) — releases 1.4 → 1.13 have been tested against fixtures and live public venue APIs, not yet end to end on real videos, so expect the parsers, matchers, the contract checklist and the cohort to need tuning on the first real run. Release-by-release detail: [CHANGELOG.md](CHANGELOG.md).
...
| Database | SQLite (WAL) + forward-only migrations (001 → 015) | Single file, crash-safe, backed up before every migration. Drizzle ORM evaluated in 0.3 (ADR-012). |
...
│       │                     dispatchLease, backup, export
│       ├── routes/           /api/* — content, research (+ dossier, sources), media, youtube, markets (+ signals, consensus, alerts, paper, contracts), subscriptions, trading, decisions (+ forecasts, limits, US paper), execution (preview/submit, intents, orders, holds, reconcile); see docs/API.md
│       └── **/*.test.ts      node:test suites (161): core, analysis, sports, markets, signals, watch/consensus, paper, trading (fake venue; the pinned SDK with stubbed fetch), provenance, contracts, forecasts, decisions, execution (E01–E12 with fault injection), media, youtube, pipelines
...
│   └── src/
│       ├── pages/            Library · Video · Predictions · Markets · Signals · Paper · Trades · Jobs · Setup
│       └── components/       PredictionDetail (quote, components, plan, evidence, dossier, history, Markets tab) · MarketLinks · ContractPanel · DossierView · SubscriptionsCard · PolymarketUsCard · TradingLimitsCard
...
Other next steps: the capped owner-run smoke test of 1.13 (one contract, quantity 1); first end-to-end run of 1.4 → 1.13 on real videos and real venue pages (parser, matcher, checklist and cohort tuning); the 1.11–1.13 exit demos on a real Polymarket US event; a paper soak long enough to attempt a production strategy qualification; the owner-run read-only account check; per-speaker creator records once diarisation exists; per-venue paper books; the remaining first-run verification in [docs/FIRST_RUN.md](docs/FIRST_RUN.md) on Windows and macOS.
