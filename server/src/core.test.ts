...
test("migrations apply in order and create the expected tables", () => {
  ...
  assert.deepEqual(tables, [
    "assessments", "component_assessments", "evidence_items", "games", "jobs", "prediction_components", "prediction_revisions", "predictions",
  ...
  assert.deepEqual(db.all<{ version: number }>("SELECT version FROM schema_migrations ORDER BY version").map((r) => r.version), [1, 2, 3, 4, 5, 6, 7]);
  db.close();
});
